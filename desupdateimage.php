    ...
    if ($_FILES["image"]["error"] === UPLOAD_ERR_NO_FILE) {
     ...
     if (!in_array($imageExtension, $validImageExtensions)) {
         echo "<script>alert('Invalid file type. Please upload an image with a valid extension');</script>";
             ...
             $uploadPath = "designerimage/" . $newImageName;
 
             if (move_uploaded_file($tempname, $uploadPath)) {
                 $des_id = $_SESSION['id'];
 
                 // Insert new category
                 $stmt = $conn->prepare("UPDATE fashion_designer SET designer_image = ? WHERE designer_id = ?");
                 $stmt->bind_param("si", $newImageName, $des_id);
                 $stmt->execute();
 
                 if ($stmt->affected_rows > 0) {
                     echo "<script> alert('Profile picture has been uploaded successfully')</script>";
                     echo "<script>window.location.href= 'desAccount.php';</script>";
                 } else {
